...
class Simulator:
    ...
    def __init__(self,
                 ...
                 dp_event_duration=config.DP_EVENTS_DURATION,
                 dp_event_generation_delay=config.DP_FEEL_EVENT,
        ...
        self.dp_event_duration=dp_event_duration
        self.dp_event_generation_delay=dp_event_generation_delay
    ...
    def __set_stepwise_discovery_mode(self):
        ...
        # if the selected routing algorithm is the QL
        if self.routing_algorithm == config.RoutingAlgorithm.QL:
            # check if the config 'STEPWISE_NODE_DISCOVERY' is equal at true
    ...
    def run(self):
        ...
        for cur_step in tqdm(range(self.len_simulation)):
            ...
            if config.DEBUG:
                print(f"[INFO] Step attuale {cur_step}")
